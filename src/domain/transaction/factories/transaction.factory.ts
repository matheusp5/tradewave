...
export class TransactionFactory {
    static services() {
        const temporaryTransactionRepository: ITemporaryTransactionRepository = new InMemoryTemporaryTransactionRepository()
        const transactionContract: ITransactionContract = new HfTransactionContract()
        const accountRepository: IAccountRepository = new InMemoryAccountRepository()
        const encrypter: IEncrypter = new JwtEncrypter()

        const transactionService = new TransactionService(transactionContract, temporaryTransactionRepository, accountRepository, encrypter)
