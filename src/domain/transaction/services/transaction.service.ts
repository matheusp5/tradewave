...
export class TransactionService {
  ...
  constructor(
    ...
    this.getTransactionsByAccountIdUseCase =
      new GetTransactionsByAccountIdUseCase(transactionContract)

    this.createTemporaryTransactionUseCase =
      new CreateTemporaryTransactionUseCase(temporaryTransactionRepository)
    this.deleteTemporaryTransactionUseCase =
      new DeleteTemporaryTransactionByIdUseCase(temporaryTransactionRepository)
    this.getAllTemporaryTransactionsByAccountIdUseCase =
      new GetAllTemporaryTransactionsByAccountIdUseCase(
        temporaryTransactionRepository
      )
    this.getTemporaryTransactionByIdUseCase =
      new GetTemporaryTransactionByIdUseCase(temporaryTransactionRepository)

    this.getAccountByEmailUseCase = new GetAccountByEmailUseCase(
      accountRepository
    )
  ...
  async createTransaction(
    request: ICreateTransactionDTO & { requester: Account }
  ) {
    ...
    const { transaction } =
      await this.createTemporaryTransactionUseCase.execute({
        payeeId: payee.id,
        payerId: payer.id,
        amount: request.amount
      })
    const confirmTransactionToken = await this.generateTokenUseCase.execute({
      sub: transaction.id,
      type: TokenType.CONFIRM_TRANSACTION
    })
  ...
  async confirmTransaction({
    token,
    requester
  }: IConfirmTransactionDTO & { requester: Account }) {
    const { sub: transactionId } = this.verifyTokenUseCase.execute({ token })

    const { transaction } =
      await this.getTemporaryTransactionByIdUseCase.execute({ transactionId })
    if (transaction.payerId !== requester.id)
      throw new BadRequestError(
        'Você não tem permissão para confirmar essa transação.'
      )
    ...
    await this.deleteTemporaryTransactionUseCase.execute({
      transactionId,
      requester
    })
  ...
  async myTransactions({ requester }: IMyTransactionsDTO) {
    const { transactions } =
      await this.getTransactionsByAccountIdUseCase.execute({
        accountId: requester.id
      })
    const { transactions: temporaryTransactions } =
      await this.getAllTemporaryTransactionsByAccountIdUseCase.execute({
        accountId: requester.id
      })
