...
describe('Delete Temporary Transaction By Id Use Case', () => {
    ...
    beforeEach(() => {
        ...
        sut = new DeleteTemporaryTransactionByIdUseCase(temporaryTransactionRepository);
    });
    ...
    it('should delete a temporary transaction by id', async () => {
        ...
        const transactionPayload = makeTransaction({ payeeId: payee.id, payerId: payer.id });
        const createTransactionResponse = await temporaryTransactionRepository.createTemporaryTransaction(transactionPayload);

        await sut.execute({ transactionId: createTransactionResponse.id, requester: payer });

        const deletedTransaction = await temporaryTransactionRepository.getTemporaryTransactionById(createTransactionResponse.id);
    ...
    it('should throw a ResourceNotFoundError if the temporary transaction does not exist', async () => {
        ...
        await expect(sut.execute({ transactionId, requester: account })).rejects.toThrow(ResourceNotFoundError);
    });
    ...
    it('should throw an UnauthorizedError if the requester is not the payer of the temporary transaction', async () => {
        ...
        const transaction = await temporaryTransactionRepository.createTemporaryTransaction(transactionPayload);

        await expect(sut.execute({ transactionId: transaction.id, requester: account })).rejects.toThrow(UnauthorizedError);
    });
});