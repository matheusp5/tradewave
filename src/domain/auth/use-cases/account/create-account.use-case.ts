...
export class CreateAccountUseCase
  implements
    IUseCase<ICreateAccountRequest, Promise<ICreateAccountUseCaseResponse>>
{
  constructor(private accountRepository: IAccountRepository) {}

  async execute(
    request: ICreateAccountRequest
  ): Promise<ICreateAccountUseCaseResponse> {
    const accountAlreadyExists = await this.accountRepository.getByEmail(
      request.email
    )
    if (accountAlreadyExists)
      throw new DuplicateResourceError('Uma conta com este email já existe.')
