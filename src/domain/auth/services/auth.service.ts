...
export class AuthService {
  ...
  async login({ email, password }: ILoginWithEmailAndPasswordDTO) {
    ...
    if (!account.password.comparePasswords(password))
      throw new UnauthorizedError(
        'E-mail ou senha não encontrados. Verifique suas credenciais.'
      )

    const accessToken = await this.generateTokenUseCase.execute({
      sub: account.id,
      type: TokenType.ACCESS
    })
    const refreshToken = await this.generateTokenUseCase.execute({
      sub: account.id,
  ...
  async refreshAuth({ refreshToken }: IRefreshAuthDTO) {
    const { sub } = await this.verifyTokenUseCase.execute({
      token: refreshToken
    })
    ...
    const newRefreshToken = await this.generateTokenUseCase.execute({
      sub,
      type: TokenType.REFRESH
    })
  ...
  async me({ requester }: { requester: Account }) {
    const balance = await this.getAccountBalanceUseCase.execute({
