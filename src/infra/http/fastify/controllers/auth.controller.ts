import { ILoginWithEmailAndPasswordDTO, IRefreshAuthDTO, IRegisterAccountDTO } from "@/domain/auth/dtos/auth.dto";
import { AuthFactory } from "@/domain/auth/factories/auth.factory";
import { FastifyReply, FastifyRequest } from "fastify";
import { AccountPresenter } from "../presenters/account.presenter";

export class AuthController {
    static async login(req: FastifyRequest<{ Body: ILoginWithEmailAndPasswordDTO }>, reply: FastifyReply) {
    ...
    static async register(req: FastifyRequest<{ Body: IRegisterAccountDTO }>, reply: FastifyReply) {
        const authService = AuthFactory.services()
    ...
    static async refreshAuth(req: FastifyRequest<{ Body: IRefreshAuthDTO }>, reply: FastifyReply) {
        const authService = AuthFactory.services()
